...
SOURCE_SUSPEND_GUARD="$SCRIPT_DIR/bin/suspend-wake-guard.sh"
...
[[ -f "$SOURCE_HELPER" && -f "$SOURCE_SUSPEND_GUARD" && -f "$SOURCE_STATUS" && -f "$SOURCE_UNINSTALL" ]] && RUNNING_FROM_SOURCE=1
...
check_dependencies() {
    ...
    need_command systemctl "systemctl is required to activate the suspend guard. Install systemd tooling, then try again."
...
install_support_files() {
    if (( RUNNING_FROM_SOURCE == 1 )); then
        ...
        as_root install -d -m 755 "$LIB_DIR" "$DOC_DIR" "$(dirname "$SUSPEND_GUARD_DEST")" "$(dirname "$SETUP_DEST")" "$(dirname "$STATUS_DEST")" "$(dirname "$UNINSTALL_DEST")"
        as_root install -m 755 "$SOURCE_HELPER" "$HELPER_DEST"
        as_root install -m 755 "$SOURCE_SUSPEND_GUARD" "$SUSPEND_GUARD_DEST"
        ...
        if [[ ! -x "$HELPER_DEST" || ! -x "$SUSPEND_GUARD_DEST" || ! -x "$STATUS_DEST" || ! -x "$UNINSTALL_DEST" ]]; then
...
write_wake_targets() {
    ...
    tmp="$(mktemp)"
    {
        echo "# HTPC Controller Wake targets"
        echo "# Generated by htpc-controller-wake-setup on $(date -Is 2>/dev/null || date)"
        echo "# One USB topology node per line. Used by the pre-suspend quiet-window guard."
        for targets in "${SEL_TARGETS[@]}"; do
    ...
    as_root install -d -m 755 "$CONFIG_DIR"
    if ! as_root install -m 644 "$tmp" "$WAKE_TARGETS_FILE"; then
        rm -f "$tmp"
        return 1
    fi
    rm -f "$tmp"
...
install_suspend_guard() {
    local tmp
    tmp="$(mktemp)"
    cat > "$tmp" <<EOF2
# Installed by HTPC Controller Wake.
# Hold only this project's configured USB wake paths quiet before suspend so
# controller/receiver shutdown traffic cannot abort the suspend transition.
[Service]
ExecStartPre=$SUSPEND_GUARD_DEST
EOF2

    as_root install -d -m 755 "$SUSPEND_DROPIN_DIR"
    if ! as_root install -m 644 "$tmp" "$SUSPEND_DROPIN"; then
        rm -f "$tmp"
        return 1
    fi
    rm -f "$tmp"

    # The drop-in is picked up on every future invocation of
    # systemd-suspend.service. No separate service needs to be enabled.
